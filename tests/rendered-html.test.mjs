...
test("serves focused work, writing, profile, and contact routes", async () => {
  ...
  assert.match(await writing.text(), /AI’s Defining Bottleneck Isn’t Intelligence\. It’s Memory\.[\s\S]*Multi-Agent Systems Work Best When Agents Know Less[\s\S]*Adaptive Agents: Why Correcting AI Is Not the Same as Teaching It[\s\S]*From Retrieved Documents to Executable Skills/);
